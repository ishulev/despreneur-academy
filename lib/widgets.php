...
class Front_Top_CTA_Widget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		echo $args['before_widget'];
		echo '<div class="row front-widget">';
...
class Signup_CTA_Widget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		// The Query
		echo $args['before_widget'];
		echo '<div class="row text-center front-widget">';
			if($big_title) {
...
class Display_Testimonials_Widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			...
			__( 'Display testimonials', 'da' ), // Name
			array( 'description' => __( 'Display testimonials for the front page', 'da' ),
	 ...
	 */
	public function widget( $args, $instance ) {
		$testimonials = get_posts( $args = array('post_type' => 'testimonial', 'posts_per_page' => 3, 'post_status' => 'publish') );
		$title = $instance['title'];
		if(!empty($testimonials)) {
			...
			foreach ( $testimonials as $testimonial ) : setup_postdata( $testimonial ); ?>
				<?php $author_link = get_post_meta( $testimonial->ID, $key = 'author_url', $single = true ); ?>
				<div class="col-md-4 text-center">
					<?php if(has_post_thumbnail( $post_id = $testimonial->ID ) ) : ?>
						<?php echo get_the_post_thumbnail( $post = $testimonial->ID, $size = 'thumbnail', $attr = array('class' => 'img-circle') ); ?>
					<?php endif; ?>
					<h4><?php echo $testimonial->post_title; ?></h4>
					<?php if ( '' !== $author_link) : ?>
						<p><?php echo $author_link; ?></p>
					<?php endif ?>
					<p><em><?php echo $testimonial->post_excerpt; ?></em></p>
				</div>
			<?php endforeach;
			echo '</div>';
			echo $args['after_widget'];
	 ...
	 */
	public function form( $instance ) {
		$title = ! empty( $instance['title'] ) ? $instance['title'] : __( 'What people are saying', 'da' );
		?>
		<p><label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title' ); ?></label> 
			<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>">
		</p>
...
} // class Display_Testimonials_Widget

class Display_Partners_Widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			...
			__( 'Display partners', 'da' ), // Name
			array( 'description' => __( 'Display partners for the front page', 'da' ),
				'title' => 'Display partners',
			) // Args
		);
	}
	 ...
	 */
	public function widget( $args, $instance ) {
		$partners = get_posts( $args = array('post_type' => 'partner', 'posts_per_page' => -1, 'post_status' => 'publish') );
		$title = $instance['title'];
		if(!empty($partners)) {
			...
			echo '<div class="row text-center front-widget">';
				echo '<h3>' . $title . '</h3>';
				foreach ( $partners as $partner ) : setup_postdata( $partner ); ?>
					<?php $partner_url = get_post_meta( $partner->ID, $key = 'partner_url', $single = true ); ?>
					<?php if (has_post_thumbnail( $post_id = $partner->ID )): ?>
						<a class="partner-logo" href="<?php echo esc_url( $url = $partner_url, $protocols, $_context ); ?>">
							<?php echo get_the_post_thumbnail( $post_id = $partner->ID, $size = 'medium', $attr ); ?>
						</a>
					<?php endif; ?>
				<?php endforeach;
			echo '</div>';
	 ...
	 */
	public function form( $instance ) {
		$title = ! empty( $instance['title'] ) ? $instance['title'] : __( 'Exclusive partner deals to kickstart your ventures', 'da' );
		?>
		<p><label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title' ); ?></label> 
			<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>">
		</p>
...
function register_custom_widgets() {
	...
	register_widget( 'Display_Testimonials_Widget' );
	register_widget( 'Display_Partners_Widget' );
}
